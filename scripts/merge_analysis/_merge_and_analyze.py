...
def load_single_file(file_path: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"로드 완료 cols: {df.columns.to_list()}")
        try:
...
def merge_shard_files(data_dir: str, output_filename: str = "raw_generated.parquet") -> pd.DataFrame:
    ...
    if os.path.exists(output_path):
        ...
        try:
            ...
            logger.info(f"기존 파일 cols: {df.columns.to_list()}")
            try:
    ...
    # 병합 전 샤드 파일들의 총 용량 계산
    total_shard_size_bytes = 0
    for shard_file in parquet_files:
        try:
            shard_size = os.path.getsize(shard_file)
            total_shard_size_bytes += shard_size
        except Exception as e:
            logger.warning(f"샤드 파일 크기 확인 실패: {shard_file}, 오류: {e}")
    
    total_shard_size_mb = total_shard_size_bytes / (1024 ** 2)
    logger.info(f"병합 전 샤드 파일 총 용량: {total_shard_size_mb:.2f} MB ({len(parquet_files)}개 파일)")
    ...
    # logprobs, generated_text 컬럼 제외하고 저장
    columns_to_exclude = ['logprobs', 'generated_text']
    columns_to_save = [col for col in merged_df.columns if col not in columns_to_exclude]
    merged_df_save = merged_df[columns_to_save].copy()
    excluded_cols = [col for col in columns_to_exclude if col in merged_df.columns]
    if excluded_cols:
        logger.info(f"{', '.join(excluded_cols)} 컬럼 제외하고 저장 (제외된 컬럼: {', '.join(excluded_cols)})")
    else:
        merged_df_save = merged_df.copy()
    
    # 결과 저장
    print(f"결과 저장 cols: {merged_df_save.columns.to_list()}")
    output_path = os.path.join(data_dir, output_filename)
    merged_df_save.to_parquet(output_path, index=False, compression="zstd")
    
    # 병합 후 저장된 파일 용량 계산
    try:
        merged_size_bytes = os.path.getsize(output_path)
        merged_size_mb = merged_size_bytes / (1024 ** 2)
        
        # 용량 비교 정보
        logger.info(f"✅ 병합 완료: {len(merged_df)}개 결과 저장")
        logger.info(f"저장 위치: {output_path}")
        logger.info(f"\n=== 용량 비교 ===")
        logger.info(f"병합 전 총 용량: {total_shard_size_mb:.2f} MB ({len(parquet_files)}개 샤드 파일)")
        logger.info(f"병합 후 용량: {merged_size_mb:.2f} MB (1개 파일)")
        if total_shard_size_bytes > 0:
            compression_ratio = (1 - merged_size_bytes / total_shard_size_bytes) * 100
            size_reduction = total_shard_size_mb - merged_size_mb
            logger.info(f"용량 감소: {size_reduction:.2f} MB ({compression_ratio:.1f}% 감소)")
            logger.info(f"압축률: {merged_size_bytes / total_shard_size_bytes * 100:.1f}% (병합 후 / 병합 전)")
    except Exception as e:
        logger.warning(f"병합 후 파일 크기 확인 실패: {output_path}, 오류: {e}")
        logger.info(f"✅ 병합 완료: {len(merged_df)}개 결과 저장")
        logger.info(f"저장 위치: {output_path}")
...
def _ensure_confidence_columns(df: pd.DataFrame, group_size: int = 10) -> pd.DataFrame:
    """
    누락된 confidence 컬럼을 logprobs로부터 계산하여 추가합니다.
    """
    required_cols = [
        'mean_group_confidence',
        'bottom_10_percent_confidence',
    ...
    for idx, lp in df['logprobs'].items():
        try:
            # numpy array 가능성 처리 및 타입 보정
            if lp is None:
                logprobs_list = []
            elif isinstance(lp, np.ndarray):
                # 토큰 x k 후보 구조를 파악하여 파이썬 리스트로 변환
                logprobs_list = lp.tolist()
            else:
                logprobs_list = lp

            # 토큰별 후보가 np.ndarray일 수 있어 다시 한 번 list로 보정
            if isinstance(logprobs_list, list):
                normalized = []
                for tok in logprobs_list:
                    if isinstance(tok, np.ndarray):
                        normalized.append(tok.tolist())
                    else:
                        normalized.append(tok)
                logprobs_list = normalized

            scores = calculator.calculate_all_confidence_scores(logprobs_list)
    ...
    # 누락된 컬럼만 추가 (이미 있으면 건너뜀)
    if 'mean_group_confidence' not in df.columns:
    ...
    if 'top_10_percent_confidence' not in df.columns:
        df['top_10_percent_confidence'] = top10_vals
    if 'highest_group_confidence' not in df.columns:
        df['highest_group_confidence'] = highest_vals

    return df


def analyze_accuracy_by_content_length(df: pd.DataFrame, output_dir: str = None, num_bins: int = 10) -> None:
    """
    </think> 이후 content 길이(토큰 근사: 공백 분리 단어 수)별 정답률 분석 및 시각화
    """
    logger.info("\n" + "="*50)
    logger.info("=== Content 길이에 따른 정답률 분석 ===")
    logger.info("="*50)
    ...
    # content 준비 (없으면 추출 시도는 상위 파이프라인에서 수행됨)
    if 'content' not in df.columns:
        logger.error("content 컬럼이 없습니다. 먼저 parse_and_verify_dataframe을 실행하세요.")
        return
    ...
    def estimate_tokens(s: Any) -> int:
        if pd.isna(s):
            return 0
        try:
            return max(0, int(len(str(s).split())))
        except Exception:
            return 0

    content_token_counts = df['content'].apply(estimate_tokens).astype(float).values
    is_correct = df[correct_col].astype(int).values

    # 유효 데이터 필터
    valid_mask = ~np.isnan(content_token_counts) & ~pd.isna(df[correct_col])
    content_vals = content_token_counts[valid_mask]
    correct_vals = is_correct[valid_mask]

    if len(content_vals) == 0:
        logger.error("분석 가능한 데이터가 없습니다.")
        return

    # 분위수 기반 엣지
    percentiles = np.linspace(0, 100, num_bins + 1)
    edges = np.unique(np.percentile(content_vals, percentiles))
    if len(edges) < 2:
        logger.error("구간을 생성할 수 없습니다 (값 다양성 부족).")
        return

    bin_accs, bin_counts, bin_labels = [], [], []
    for i in range(len(edges) - 1):
        left, right = edges[i], edges[i + 1]
        if i < len(edges) - 2:
            mask = (content_vals >= left) & (content_vals < right)
        else:
            mask = (content_vals >= left) & (content_vals <= right)
        cnt = mask.sum()
        if cnt > 0:
            acc = correct_vals[mask].mean()
            bin_accs.append(float(acc))
            bin_counts.append(int(cnt))
            bin_labels.append(f"{int(left)}-{int(right)}")

    logger.info(f"총 유효 샘플: {len(content_vals)}")
    for lbl, acc, cnt in zip(bin_labels, bin_accs, bin_counts):
        logger.info(f"  [{lbl}] 정답률: {acc:.3f} ({cnt}개)")
    ...
    try:
        fig, ax1 = plt.subplots(figsize=(11, 6))
        x = np.arange(len(bin_labels))
        ax1.bar(x, bin_accs, color='#54A24B', alpha=0.85, label='Accuracy')
        ax1.set_ylim(0, 1)
        ax1.set_ylabel('Accuracy')
        ax1.set_xlabel('Content token count bins')
        ax1.set_title('Accuracy by Content Length (after </think>)')
        ax1.set_xticks(x)
        ax1.set_xticklabels(bin_labels, rotation=30, ha='right')
        ax1.grid(True, axis='y', alpha=0.3)

        ax2 = ax1.twinx()
        ax2.plot(x, bin_counts, color='#E45756', marker='o', linewidth=2, label='Count')
        ax2.set_ylabel('Count')
        ...
        plot_path = os.path.join(output_dir, 'accuracy_by_content_length.png')
        plt.tight_layout()
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        logger.info(f"시각화 저장: {plot_path}")
...
def analyze_single_file(file_path: str, output_dir: str = None) -> None:
    ...
    # 2.1 confidence 컬럼 보강 (누락 시 logprobs로부터 계산)
    df = _ensure_confidence_columns(df)

    # 2.1 confidence 컬럼 보강 (누락 시 logprobs로 계산)
    df = _ensure_confidence_columns(df)
    ...
    # 8. Content 길이에 따른 정확도 분석 (</think> 이후)
    analyze_accuracy_by_content_length(df, output_dir)
...
def main(data_dir: str = None, output_filename: str = "raw_generated.parquet", single_file: str = None) -> None:
    ...
    if 'reasoning_content' not in df.columns or 'content' not in df.columns or 'final_answer' not in df.columns or 'is_correct_math_verifier' not in df.columns:
        ...
        if 'logprobs' in df.columns:
            columns_to_save = [col for col in df.columns if col != 'logprobs']
            df_save = df[columns_to_save].copy()
            logger.info(f"logprobs 컬럼 제외하고 저장 (제외된 컬럼: logprobs)")
        else:
            df_save = df.copy()
        
        # 파싱된 결과 저장
        output_path = os.path.join(data_dir, output_filename)
        print(f"파싱된 데이터 저장: {output_path}")
        print(df_save.head(5))
        df_save.to_parquet(output_path, index=False, compression="zstd")
        logger.info(f"파싱된 데이터 저장: {output_path}")
        print(f"파싱된 데이터 저장 완료: {output_path}")
    else:
        logger.info("이미 파싱된 데이터입니다.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="raw_generated.parquet", 
                       help="출력 파일명 (병합 모드)")
