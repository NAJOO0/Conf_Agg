...
- vLLM API 자동 감지 및 호환성 지원
  * V1 API: per-request generate(prompt, sampling_params, request_id)
  * Legacy API: add_request() + generate()/get_next_response()
- 백프레셔 제어 (Semaphore)
...
def _use_per_request_generate(engine: AsyncLLMEngine) -> bool:
    """
    엔진이 per-request generate API를 지원하는지 자동 판별
    V1: generate(prompt, sampling_params, request_id)
    Legacy: add_request() + generate() 또는 get_next_response()
    """
    try:
        from inspect import signature
        sig = signature(engine.generate)
        params = sig.parameters
        # V1 스타일: prompt, sampling_params, request_id가 모두 파라미터에 있어야 함
        return all(k in params for k in ("prompt", "sampling_params", "request_id"))
    except Exception as e:
        logger.warning(f"generate 시그니처 검사 실패: {e}, fallback to legacy API")
        return False
...
async def run_one_request_v1(
    ...
    """
    V1 API: 단일 요청을 per-request generate로 처리
    """
    await semaphore.acquire()
    ...
    try:
        # V1 공개 API: 매 요청마다 generate(prompt, sampling_params, request_id)
        async for request_output in engine.generate(
            prompt=prompt,
            sampling_params=sampling_params,
            request_id=request_id,
            ...
            # Shutdown 체크
            if shutdown_event.is_set():
                logger.warning(f"[Shard {shard_id}] Shutdown 신호 감지, 요청 처리 중단")
                break
            
            # 요청 처리
            stats = await process_request_output(
                request_output=request_output,
                request_id_to_index={request_id: index},
                problems=problems,
                prompt_token_counts=prompt_token_counts,
                confidence_calculator=confidence_calculator,
                file_handlers=file_handlers,
        ...
        # 진행 상황 업데이트
        progress_counter["finished"] += 1
        progress_counter["appended"] += total_appended
        progress_counter["skipped"] += total_skipped
        
        # 주기적 로깅
        if snapshot_every > 0 and progress_counter["finished"] % snapshot_every == 0:
            await log_progress(
                shard_id=shard_id,
                finished_requests=progress_counter["finished"],
                total_requests=progress_counter["total"],
                total_appended=progress_counter["appended"],
                total_skipped=progress_counter["skipped"],
            )
    
    except Exception as e:
        logger.error(f"[Shard {shard_id}] 요청 처리 실패 (ID: {request_id}): {e}", exc_info=True)
    
    finally:
        semaphore.release()
    
    return {"appended": total_appended, "skipped": total_skipped}
...
async def main_worker_async(cfg: DictConfig, args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # API 버전 자동 판별
        use_per_request_api = _use_per_request_generate(engine)
        app_logger.info(f"[Shard {args.shard_id}] vLLM API 모드: {'V1 (per-request generate)' if use_per_request_api else 'Legacy (add_request + generate/get_next_response)'}")
        ...
        with FileHandlers(jsonl_path, parquet_path, flush_every) as file_handlers:
            # 15. API 버전에 따라 실행 방식 분기
            if use_per_request_api:
                # V1 방식: per-request generate
                app_logger.info(f"[Shard {args.shard_id}] V1 per-request 방식으로 요청 처리 시작...")
                ...
                # 모든 요청에 대한 태스크 생성
                tasks = []
                for i, prompt in enumerate(my_texts):
                    request_id = f"req_{args.shard_id}_{i}"
                ...
                if exceptions:
                    app_logger.error(f"{len(exceptions)}개 요청 처리 중 오류 발생")
                    for exc in exceptions[:5]:  # 처음 5개만 출력
                        app_logger.error(f"  오류: {exc}", exc_info=exc)
                ...
                app_logger.info(f"✅ [Shard {args.shard_id}] V1 방식 처리 완료")
                ...
                # Legacy 방식: add_request + generate/get_next_response
                app_logger.info(f"[Shard {args.shard_id}] Legacy 방식 (Feeder + Collector)으로 처리 시작...")
                ...
                if isinstance(collector_result, dict):
                    ...
                    app_logger.info(f"✅ [Shard {args.shard_id}] Legacy 방식 처리 완료")
                    app_logger.info(f"저장 위치: {parquet_path}")
                    app_logger.info(
                        f"처리 완료: {finished_requests}개 요청, "
                        f"{total_appended}행 저장, {total_skipped}행 스킵"
